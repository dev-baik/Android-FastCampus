...
internal class FaceAnalyzer(
    ...
    private var heightSacleFactor = 1F
    ...
    private val successListener = OnSuccessListener<List<Face>> { faces ->
        ...
        if (face != null) {
            if (detectStatus == FaceAnalyzerStatus.UnDetect) {
                ...
                listener?.detectProgress(180F, "얼굴 인식이 완료되었습니다.")
                listener?.stopDetect()
                detector.close()
            }
            calDetectSize(face)
    ...
    override fun analyze(image: ImageProxy) {
        ...
        heightSacleFactor = preview.height.toFloat() / image.width
        detectFaces(image)
    }
    ...
    private fun Int.translateY() = toFloat() * heightSacleFactor
